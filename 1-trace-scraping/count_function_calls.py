...
def parse_file(filename, foldername):
    global total, failed, python2, failed_dirs, index, function_calls
    print(filename)
    total += 1
    try:
        try:
    ...
    parse_tree(tree, foldername, filename, function_calls)
...
def parse_string(string_to_parse):
    ...
    return parse_tree(tree, foldername, filename, function_calls)

def parse_tree(tree, foldername, filename, function_calls):
    function_mapping = {}
...
def count_function_calls():
    global total, failed, python2, failed_dirs, i, function_calls, filenames, index
    total = 0
    failed = 0
    python2 = 0
    function_calls = {}
    ...
    for filename, foldername in filenames:
        parse_file(filename, foldername)
    ...
    output_filename = os.path.join("function_counts", f"{array_index}-data.pkl")
